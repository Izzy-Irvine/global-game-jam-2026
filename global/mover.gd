...
func _process(delta: float) -> void:
	time += delta
	position.x = base_position.x + sin((time + move_x_offset * PI) * move_x_speed) * move_x_amplitude
	position.y = base_position.y + sin((time + move_y_offset * PI) * move_y_speed) * move_y_amplitude
